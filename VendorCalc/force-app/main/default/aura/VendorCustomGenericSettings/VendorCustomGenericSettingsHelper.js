({
    ...
    setGoodsTypeAction_helper: function (c, e, h, selectedProgramPartnerId, isCopy) {
        try {
            console.log('isCopy', isCopy);
            if (!$A.util.isUndefinedOrNull(selectedProgramPartnerId)) {
                console.log('selectedCopyPriceSettings', c.get('v.selectedCopyPriceSettings'));
                ...
                action.setCallback(this, function (r) {
                    if (r.getState() === 'SUCCESS') {
                        ...
                        if (!$A.util.isUndefinedOrNull(storedResponse)) {
                            ...
                            if (isCopy) {
                                c.set("v.selectedCopyPriceSettings.VEN3__Goods_Type__c", c.get('v.selectedCopyPriceSettings.VEN3__Goods_Type__c'));
                            } else {
                ...
                c.set("v.PicklistOptionsGoodsType", goodsTypeOption);
                if (isCopy) {
                    c.set("v.selectedCopyPriceSettings.VEN3__Goods_Type__c", goodsTypeOption[0].value);
                } else {
                    c.set("v.calcSettings.VEN3__Goods_Type__c", goodsTypeOption[0].value);
                }
                h.hideSpinner_Helper(c);
    ...
    copyProgramPricing_Helper: function (c, e, h) {
        ...
        if (!$A.util.isUndefinedOrNull(selectedSettingId) && !$A.util.isUndefinedOrNull(allSettings)) {
            ...
            if (!$A.util.isUndefinedOrNull(selectedSettingObj)) {
                console.log('selectedSettingObj', selectedSettingObj);
    ...
    checkExistingProgramPricingSetting: function (c, e, h) {
        ...
        allExistingPriceSettings.forEach(function (elem) {
            if (!$A.util.isUndefinedOrNull(elem.VEN3__Goods_Type__c) && !$A.util.isUndefinedOrNull(elem.VEN3__Term_months__c) && !$A.util.isUndefinedOrNull(elem.VEN3__Facility_Type__c) && !$A.util.isUndefinedOrNull(elem.VEN3__Asset_Type__c) && !$A.util.isUndefinedOrNull(elem.VEN3__Program_Partner__c)) {
                if(elem.VEN3__Program_Partner__c === selectedSettings.VEN3__Program_Partner__c && elem.VEN3__Goods_Type__c === selectedSettings.VEN3__Goods_Type__c && elem.VEN3__Term_months__c === selectedSettings.VEN3__Term_months__c && elem.VEN3__Facility_Type__c === selectedSettings.VEN3__Facility_Type__c && elem.VEN3__Asset_Type__c === selectedSettings.VEN3__Asset_Type__c){
                    selectedSettings.push(elem);
                }
            }
        });
        ...
        var selectedSettings = c.get("v.selectedCopyPriceSettings");
        var action = c.get("c.selectExistingSetting");
        action.setParams({
            "selectedCopySetting": JSON.stringify(selectedSettings),
        });
        action.setCallback(this, function (response) {
            var state = response.getState();
            if (state === "SUCCESS") {
